...
class MyWidget(QtWidgets.QWidget):
    ...
    def open_gallery(self):
        if os.path.exists(r'result\classification') and len(os.listdir(r'result\classification'))>0:
...
class GalleryWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.font = QtGui.QFont("Bodoni MT", 20, QtGui.QFont.Bold)
        self.classfication_path = r'result\classification'
    ...
    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        ...
        for key, value in self.imgs_to_class.items():
            self.move_image(key, value)
        
        print('ALL CHANGES ARE SAVED')
    
    def move_image(self, src, to_label):
        # src = os.path.join(base_dir, str(from_label), image_name)
        p = Path(src)
        from_label = p.parent.name 
    ...
    def find_dirs_with_classification(self):
        ...
        if os.path.exists(self.classfication_path):
            ...
            for cl in class_dirs:
                ...
                for img in imgs:
                    imgs_to_class[os.path.join(cl, img)] = class_
    ...
    def init_ui(self):
        ...
        # self.class_labels = ['ad', 'not_ad'] # step4
    ...
    def label_change(self, index):
        ...
        curr_img = list(self.imgs_to_class.keys())[self.counter]
        self.imgs_to_class[curr_img] = self.class_labels[index_label]


    def show_image(self):
        img_path = list(self.imgs_to_class.keys())[self.counter]#os.path.join(self.INPUT_DIR, self.image_list[self.counter])
        ...
        self.info_label.setText(
            f"Image {self.counter + 1} of {len(self.imgs_to_class.items())}"
        )
        ...
        label = self.imgs_to_class[img_path]
